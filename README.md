# Rhetorix Official Website  
The official platform for **Rhetorix**, a youth-led initiative founded in 2025 to uplift India’s overlooked expressive disciplines, including public speaking, debates, group discussions, elocution, and cultural articulation.  
This website powers **Prastavana Conclave**, the premier committee-based debating competition hosted at Prelude Public School, Agra.

---

## 🚀 Features
- **Full Frontend Website** built with HTML, CSS, and JavaScript  
- **Firebase Integration**  
  - Firestore Database   
  - Hosting  
- **Dynamic Registration System**  
- Real-time committee preference and portfolio input  
- **Public Eye Matrix access** (Google Sheets live dashboard)  
- Optimized for mobile, tablet, and desktop  
- Multiple sections: Home, About, Mission, Committees, Events, Registration, Contact  
- Custom premium design (Red × Gold × Black theme)

---

## 🛠️ Tech Stack

### **Frontend**
- HTML5  
- CSS3  
- JavaScript (Vanilla)    

### **Backend / Infrastructure**
- **Firebase**
  - Firestore Database  
  - Firebase Hosting  
- Google Drive API (indirectly via form for payment screenshot uploads)

  ---
